            ...
            <div class="container-fluid pt-4 px-4 ">
                ...
                <div class="container mt-5">
                    <div class="custom-div"
                        ...
                        <p>
                        <h4 style="color: #33CCFF	;"><i class="fas fa-user"></i> Bệnh nhân:
                            {{ $patientRecords->prname }}</h4>
                        </p>
                        <p><i class="fas fa-birthday-cake"></i> Ngày sinh: <b
                                style="color: #003553">{{ $patientRecords->birthday }}</b></p>
                        <p><i class="fas fa-phone-alt"></i> Số điện thoại: <b
                                style="color: #003553">{{ $patientRecords->phonenumber }}</b></p>
                        <p><i class="fas fa-venus-mars"></i> Giới tính: <b
                                style="color: #003553">{{ $patientRecords->gender == 'male' ? 'Nam' : ($patientRecords->gender == 'female' ? 'Nữ' : 'Khác') }}</b>
                        </p>
                        <p><i class="fas fa-map-marked-alt"></i> Địa chỉ: <b
                                style="color: #003553">{{ $patientRecords->address }}</b></p>
                    ...
                    @isset($updatekq)
                        <div class="custom-div mt-4"
                            ...
                            @if ($updatekq->image != null)
                                <img class="img-fluid" style="border:2px solid #33CCFF; width: 400px; height: auto;"
                                    src="{{ asset('image/' . $updatekq->image) }}" alt="">
                            @else
                            @endif
                        </div>
                        <div class="custom-div mt-4"
                            ...
                            <form action="{{ url('capnhatkq/' . $updatekq->id_result) }}" method="post"
                                ...
                                <div class="d-flex justify-content-end">
                                    ...
                                    <button type="submit" style="margin-right:20px;" class="btn btn-primary"><i
                                            class="far fa-edit"></i> Cập nhật</button>
                                    @isset($dk)
                                        <a href="{{ route('themdonthuoc', ['id' => $dk]) }}" class="btn btn-primary"><i
                                                class="fas fa-first-aid"></i> Đơn thuốc </a>
                                    @endisset
                                </div>
                        </div>
                    @endisset
